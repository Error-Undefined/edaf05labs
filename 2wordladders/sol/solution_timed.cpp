...
int main()
{
  ...
  std::unordered_map<std::string, int> lookupTable; //we store the words as unique integers instead of strings.

  std::list<int> *graph[words]; //the graph

  {
    std::unordered_map<int, std::string> reverseLookupTable;
    int i = -1;
